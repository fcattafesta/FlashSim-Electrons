...
auto match_gen_to_reco(
    ROOT::VecOps::RVec<float> &gen_pt, ROOT::VecOps::RVec<float> &gen_eta,
    ROOT::VecOps::RVec<float> &gen_phi, ROOT::VecOps::RVec<float> &gen_chg,
    ROOT::VecOps::RVec<float> &reco_pt, ROOT::VecOps::RVec<float> &reco_eta,
    ROOT::VecOps::RVec<float> &reco_phi, ROOT::VecOps::RVec<int> &reco_chg) {
  /* Matches gen level objects to reco level objects
          and returns a mask of the matched objects
  */
  auto size_outer = gen_pt.size();
  auto size_inner = reco_pt.size();

  ROOT::VecOps::RVec<int> mask;
  mask.reserve(size_outer);

  for (size_t i = 0; i < size_outer; i++) {
    mask.emplace_back(0);
    for (size_t j = 0; j < size_inner; j++) {
      auto deta = gen_eta[i] - reco_eta[j];
      auto dphi = TVector2::Phi_mpi_pi(gen_phi[i] - reco_phi[j]);
      auto dr = TMath::Sqrt(deta * deta + dphi * dphi);
  ...
  auto matched =
      pre.Define("isLastCopy",
          ...
          .Define("ClosestJet_EncodedHadronFlavour_light",
                  ...
                  {"CleanGenJet_eta", "CleanGenJet_phi", "GenElectron_eta",
                   "GenElectron_phi", "CleanGenJet_hadronFlavour"})
          .Define("GenElectron_isReco", match_gen_to_reco,
